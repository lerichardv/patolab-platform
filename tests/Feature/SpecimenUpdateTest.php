...
test('when creating a specimen, if the CAI range invoice number is taken, it tries with the next one', function () {
    Carbon::setTestNow('2026-07-15');
    ...
    // Create an invoice with full_invoice_number = 000-001-01-00000005 (to test checking first)
    Invoice::create([
        ...
        'total' => 500.00,
        'total_paid' => 500.00,
        'invoice_file' => 'invoices/existing5.pdf',
    ]);

    // Create an invoice with full_invoice_number = 000-001-01-00000006
    Invoice::create([
    ...
    // Call store specimen
    $response = $this->actingAs($user)->post(route('specimens.store'), [
        'customer' => $customer->id,
        'specimen_type' => $type->id,
        'specimen_type_examination' => $examination->id,
        'specimen_category' => $category->id,
    ...
    // Check that a new invoice was created with 00000007 (since 00000006 was taken)
    $newInvoice = Invoice::where('full_invoice_number', '000-001-01-00000007')->first();
    expect($newInvoice)->not->toBeNull();
...
test('when creating a specimen, if the CAI range last_used_number is NOT taken, it uses it directly', function () {
    Carbon::setTestNow('2026-07-15');
    ...
    // Note: 000-001-01-00000005 is NOT in DB
    ...
    // Check that invoice was created with 00000005
    $newInvoice = Invoice::where('full_invoice_number', '000-001-01-00000005')->first();
    expect($newInvoice)->not->toBeNull();

    $caiRange->refresh();
    // It gets incremented by 1 after successful creation
    expect($caiRange->last_used_number)->toEqual(6);
});
