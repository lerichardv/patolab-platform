...
@section('content')
    @php
        $requiresReport = $requiresReport ?? ($specimen->type->requires_report ?? true);
    @endphp
    ...
    @else
        <h2 style="color: #16a34a; margin-top: 0; font-size: 22px;">¡Su Muestra ha sido Finalizada!</h2>
        
        <p>Hola, <strong>{{ $customer->name }}</strong>,</p>
        
        <p>Le informamos que el análisis y procesamiento de su muestra con código <strong>{{ $specimen->sequence_code }}</strong> ha finalizado exitosamente.</p>
        
        <p>Puede visualizar el historial y estado de su muestra en tiempo real a través de nuestro portal digital:</p>
        
        <div style="text-align: center; margin: 30px 0;">
            <a href="{{ $statusUrl }}" class="btn" style="background-color: #16a34a; color: #ffffff; text-decoration: none; box-shadow: 0 4px 6px -1px rgba(22, 163, 74, 0.2);">Ver Estado en Portal</a>
        </div>
    @endif
