    ...
    return (
        <div className="fixed right-6 bottom-6 z-50 flex flex-col items-end">
                ...
                <div className="mb-4 flex h-[520px] w-[350px] flex-col overflow-hidden rounded-2xl border border-border/80 bg-card text-card-foreground shadow-2xl transition-all duration-300 ease-in-out sm:w-[400px]">
                    ...
                    <div className="flex items-center justify-between bg-gradient-to-r from-accent to-primary px-4 py-4 text-white shadow-md">
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                <h3 className="flex items-center gap-1 text-sm font-semibold tracking-wide">
                                    <span>PatoLab</span>
                    ...
                    {/* Messages Body */}
                    <div className="flex-1 scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent space-y-4 overflow-y-auto bg-muted/20 p-4">
                        {messages.map((message, index) => (
                            ...
                            >
                                {message.role === 'assistant' && (
                                    <div className="flex h-7.5 w-7.5 shrink-0 items-center justify-center self-start rounded-full border border-border/60 bg-white p-1 shadow-sm">
                                        <img
                                            src="/images/patolab-isotipo.png"
                                            alt="AI"
                                            className="h-full w-full object-contain"
                        ...
                        <div className="flex flex-col gap-1.5">
                            <div className="flex items-center gap-2">
                                <Input
                                    ...
                                    disabled={isLoading}
                                    className="h-9 flex-1 rounded-xl border-none bg-muted/40 px-3 py-2 text-sm shadow-none focus-visible:ring-1 focus-visible:ring-primary"
                                />
                                <Button
                                    type="submit"
                                    size="icon"
                                    className="h-9 w-9 shrink-0 rounded-xl bg-gradient-to-tr from-accent to-primary text-white shadow-md shadow-indigo-500/10 transition-all hover:opacity-90 active:scale-95"
                                    disabled={!input.trim() || isLoading}
                                >
                                    <Send className="h-4 w-4" />
                                </Button>
                            </div>
                            <div className="mt-0.5 text-center text-[9px] font-light text-muted-foreground/75">
                                Asistente PatoLab AI • La información puede ser
                                inexacta.
                            </div>
                        </div>
                    </form>
            ...
            <Button
                ...
                size="icon"
                className={`h-14 w-14 rounded-full shadow-lg transition-all duration-300 hover:scale-105 focus:outline-none active:scale-95 ${
                    isOpen
