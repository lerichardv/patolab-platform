...
export interface SectionsOrderElement {
    key: string;
    order: number;
    active: boolean;
}
...
interface Props {
    ...
    users?: User[]; // Admin mode if present
    onSuccess: () => void;
    storeAction: () => { url: string };
    updateAction: (id: number) => { url: string };
}
...
export default function TemplateForm({
    ...
    const getMergedSectionsOrder = (
        savedSections: SectionsOrderElement[] | null | undefined,
    ) => {
        if (
            !savedSections ||
        ...
        const loaded = [...savedSections].sort((a, b) => a.order - b.order);
    ...
    const getMergedHeadingsToggles = (
        savedToggles: Record<string, boolean> | null | undefined,
    ) => {
        return {
            ...defaultHeadingsToggles,
            ...(savedToggles || {}),
        };
    };
    ...
    useEffect(() => {
        ...
        setSectionsOrder(getMergedSectionsOrder(template?.sections_order));
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (template) {
            put(updateAction(template.id).url, options);
        } else {
            post(storeAction().url, options);
        }
    };

    // Prepare users options for combobox if users are passed
    ...
    return (
        <>
            ...
            >
                ...
                <div className="min-h-0 flex-1 overflow-y-auto bg-muted/10 px-2 px-4 py-6">
                                                ...
                                                {(provided, snapshot) => {
                                                    const child = (
                                                        <div
                                                            ...
                                                            className={cn(
                                                                'space-y-2 rounded-xl shadow-xs transition-all duration-200',
                                                                snapshot.isDragging
                                                                    ? 'w-[720px] max-w-[90vw] rotate-1'
