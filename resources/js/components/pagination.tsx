...
export function Pagination({ links, meta }: PaginationProps) {
    if (links.length <= 3) return null; // Don't show if there's only one page
        ...
        <div className="flex flex-col items-center justify-between gap-4 py-4 md:flex-row">
            {meta && (
                <p className="text-sm text-muted-foreground">
                    Mostrando <span className="font-medium">{meta.from || 0}</span> a <span className="font-medium">{meta.to || 0}</span> de <span className="font-medium">{meta.total}</span> resultados
                </p>
            )}
