...
export default function AsyncCustomerCombobox({
    ...
    const [knownCustomers, setKnownCustomers] = React.useState<
    ...
    const displayValue =
        query || (allowClear && !value ? '' : (selectedCustomer?.name ?? ''));
        ...
        <div ref={containerRef} className="relative w-full">
            <div className="relative">
                ...
                <input
                    ...
                    disabled={disabled}
                    className={cn(
                        'flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 pr-8 pl-8 text-sm shadow-sm transition-colors',
                ...
                {loading ? (
                    ...
                    <button
                        ...
                        className="absolute top-1/2 right-1 -translate-y-1/2 rounded-sm p-1 text-muted-foreground opacity-70 transition-opacity hover:opacity-100"
                    >
