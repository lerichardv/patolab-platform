...
export function ImportWizard({
	...
	}, [step, parsedData, mapping, fileName, currentIndex, successCount, errorCount, importLogs, storageKey]);
	...
	return (
			...
			<div className="flex items-center justify-between rounded-xl border bg-muted/40 p-4">
				{[
					...
					<div key={s.number} className="flex items-center gap-2">
						<div
							className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold transition-all ${step === s.number
				...
				>
					...
					{isParsing ? (
						...
						<label
							...
							className={`flex w-full max-w-md cursor-pointer flex-col items-center gap-4 rounded-lg border p-6 text-center transition-all ${dragActive
						...
						>
							...
							<div>
								...
								<p className="mt-1 text-sm text-muted-foreground">
									O haga clic para buscar en su equipo (.csv,
									.xlsx, .xls, .ods)
								</p>
							</div>
							<Button
						...
						<div className="space-y-6">
							...
							<div className="space-y-3 rounded-xl border bg-card p-6">
								...
								<span className="block text-center text-[11px] text-muted-foreground">
									Mostrando las primeras 3 filas con el mapeo
									actual.
								</span>
							</div>
						</div>
				...
				<div className="flex flex-col gap-6 rounded-xl border bg-card p-6">
					...
					{paused && currentIndex > 0 && (
						<div className="flex items-center gap-3 rounded-lg border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-700 dark:text-amber-400">
							...
							<span>
								Se restauró el progreso anterior ({currentIndex} de{' '}
								{parsedData.rows.length} filas procesadas). Haga clic en{' '}
						...
						<div className="h-[250px] space-y-2 overflow-y-auto rounded-lg border bg-muted/20 p-4 font-mono text-xs">
							{filteredLogs.length === 0 ? (
								...
								filteredLogs.map((log, index) => (
									<div
										key={index}
										className={`flex items-start gap-2 border-b border-border/50 py-1.5 last:border-0 ${log.status === 'success'
			...
			{step === 4 && parsedData && (
				<div className="mx-auto flex max-w-2xl flex-col items-center gap-6 rounded-xl border bg-card p-8 text-center shadow-md">
					...
					<div className="flex w-full flex-col justify-center gap-3 sm:flex-row">
						...
						<Button
							variant="outline"
							onClick={() => { clearStorage(); router.visit(redirectUrl); }}
							className="w-full sm:w-auto"
						>
							Ir a la Lista
						</Button>
		...
		<AlertDialog open={showResetDialog} onOpenChange={setShowResetDialog}>
			<AlertDialogContent>
				<AlertDialogHeader>
					...
					<AlertDialogDescription>
						Tiene progreso guardado en este asistente de importación
						{currentIndex > 0 && (
							<> ({currentIndex} de {parsedData?.rows.length ?? 0} filas procesadas)</>
						)}
						. Si cambia el archivo,{' '}
						<strong>perderá todo el progreso y el mapeo de columnas</strong>
		...
		<AlertDialog open={showCancelDialog} onOpenChange={setShowCancelDialog}>
			<AlertDialogContent>
				<AlertDialogHeader>
					<AlertDialogTitle>¿Cancelar importación?</AlertDialogTitle>
					<AlertDialogDescription>
						{parsedData && currentIndex < parsedData.rows.length && (
							<>
								Quedan{' '}
								<strong>
									{parsedData.rows.length - currentIndex} filas
								</strong>{' '}
								por procesar. Si cancela ahora,{' '}
								<strong>esas filas no serán importadas</strong>.
								Las filas ya procesadas ({currentIndex}) se mantienen
								guardadas.
							</>
						)}
					</AlertDialogDescription>
