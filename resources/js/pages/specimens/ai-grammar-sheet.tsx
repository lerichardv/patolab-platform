	...
	useEffect(() => {
		if (open) {
			...
			if (text.length > 3000) {
				...
				toast.warning('El texto seleccionado supera el límite. Se han recortado los primeros 3000 caracteres.');
			}
			...
			>
				...
				<div className="flex-1 space-y-6 overflow-y-auto p-6">
					<div className="space-y-2">
						...
						<div className="max-h-[200px] overflow-y-auto rounded-xl border border-border/80 bg-muted/40 p-4 text-md leading-relaxed text-gray-800 shadow-inner whitespace-pre-wrap">
							{originalText}
						</div>
						<div className="flex justify-between items-center text-xs mt-1 px-1">
							<span className="text-muted-foreground">Máximo 3000 caracteres</span>
							<span className={cn(
								"font-medium transition-colors",
								(selectedText?.length || 0) > 3000 ? "text-red-500 font-semibold" : "text-muted-foreground"
							)}>
						...
						{tempCorrectedText ? (
							<div className="space-y-3">
								<div className="flex rounded-lg bg-muted p-1">
									<button
										...
										className={cn(
											"flex flex-1 items-center justify-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-semibold transition-all cursor-pointer",
											aiViewMode === 'diff'
								...
								{aiViewMode === 'diff' ? (
									<div className="min-h-[180px] w-full rounded-xl border border-border bg-muted/10 p-4 text-md leading-relaxed shadow-inner overflow-y-auto whitespace-pre-wrap">
										{diffWords(originalText, tempCorrectedText).map((part, index) => {
											if (part.type === 'added') {
												return (
													<span
														key={index}
														className="bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 px-0.5 rounded border border-emerald-500/20 font-medium"
													>
														{part.text}
													</span>
												);
											...
											if (part.type === 'removed') {
												return (
													<span
														key={index}
														className="bg-red-500/15 text-red-700 dark:text-red-400 px-0.5 rounded border border-red-500/20 line-through decoration-red-500"
													>
														{part.text}
													</span>
												);
									...
									<Textarea
										className="min-h-[180px] w-full resize-none text-md"
							...
							<div className="space-y-4 rounded-xl border border-dashed border-border bg-muted/20 p-6 text-center">
								<div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-primary/10 text-primary">
									<Sparkles className={cn("h-5 w-5", isCorrecting && "animate-pulse")} />
								</div>
								<div className="space-y-1">
									...
									<p className="text-xs text-muted-foreground">
										Analiza el texto seleccionado para corregir ortografía, gramática y coherencia clínica.
									</p>
								</div>

								{correctionError && (
									<p className="text-xs font-medium text-destructive bg-destructive/10 py-1.5 px-3 rounded-lg">
										{correctionError}
									</p>
								)}
								...
								<Button
									type="button"
									onClick={() => fetchGrammarCorrection(originalText)}
								...
								>
									{isCorrecting ? (
										...
										<>
											<Sparkles className="h-4 w-4" />
											<span>Generar corrección con IA</span>
										</>
									)}
								</Button>
							</div>
				...
				<SheetFooter className="mt-auto flex flex-col gap-2 border-t border-border bg-muted/10 p-6 sm:flex-row">
					{!isCorrecting &&
						...
						<>
							<Button
								type="button"
								onClick={() => onReplace(tempCorrectedText, originalText)}
