...
import SpecimenBulkPathologistSkeleton from './specimen-bulk-pathologist-skeleton';
...
export default function SpecimenBulkPathologistSheet({
    ...
    const [localSpecimens, setLocalSpecimens] = useState<any[] | null>(null);
    const [prevSelectedSpecimens, setPrevSelectedSpecimens] =
        useState(selectedSpecimens);
    const [isAssigning, setIsAssigning] = useState(false);
    const hasMutatedRef = useRef(false);

    if (selectedSpecimens !== prevSelectedSpecimens) {
        setPrevSelectedSpecimens(selectedSpecimens);
        setLocalSpecimens(null);
    }

    const currentSpecimens = localSpecimens ?? selectedSpecimens;
    ...
    const availablePathologists = useMemo(() => {
        return resolvedPathologists.filter((p) => {
            const assignedCount = currentSpecimens.filter((specimen) =>
        ...
        try {
            ...
            if (response.data.specimens) {
                const updatedMap = new Map(
                    response.data.specimens.map((s: any) => [s.id, s]),
                );
                setLocalSpecimens((prev) =>
                    (prev ?? selectedSpecimens).map((s) => {
                        const updated = updatedMap.get(s.id) as any;
            ...
            toast.success(
                response.data.message ||
                    'Patólogo asignado en lote correctamente',
            );
    ...
    const handleToggleAccess = async (
        ...
        const assignedSpecimens = currentSpecimens.filter((specimen) =>
        ...
        // Optimistic update
        setLocalSpecimens((prev) =>
            (prev ?? selectedSpecimens).map((specimen) => {
                if (!specimen.users?.some((u: any) => u.id === userId)) {
                    return specimen;
                }

                return {
                    ...specimen,
                    users: specimen.users.map((u: any) =>
                        u.id === userId
                            ? {
                                  ...u,
                                  pivot: {
    ...
    const handleUnassign = async (userId: number) => {
        ...
        try {
            ...
            toast.success(
                response.data.message ||
                    'Patólogo desasignado en lote correctamente',
            );
        } catch (error: any) {
            const message =
                error.response?.data?.message ||
        ...
        <Sheet open={open} onOpenChange={handleOpenChange}>
            <SheetContent className="w-full overflow-y-auto sm:max-w-[90vw] md:max-w-[650px] lg:max-w-[750px]">
                ...
                {isLoading && !formData && pathologists.length === 0 ? (
                    <SpecimenBulkPathologistSkeleton />
                ) : (
                        ...
                        <div className="space-y-3.5 rounded-lg border border-border/60 bg-muted/20 p-4 shadow-sm">
                            ...
                            {availablePathologists.length > 0 ? (
                                ...
                                <div className="rounded-md border border-dashed bg-muted/40 p-3.5 text-center text-xs text-muted-foreground">
                                    ...
                                    asignados a todas las muestras
                                    seleccionadas.
                                </div>
                            )}
                        </div>
                                ...
                                {assignedPathologists.length > 0 ? (
                                    ...
                                    <div className="flex h-full flex-col items-center justify-center space-y-2 px-4 py-12 text-center">
                                        ...
                                        <p className="max-w-xs text-xs text-muted-foreground">
                                            Ninguno de las muestras
                                            seleccionadas tiene patólogos
                                            asignados. Utilice el selector de
                                            arriba para añadir uno.
                                        </p>
                                    </div>
                                )}
                            </div>
