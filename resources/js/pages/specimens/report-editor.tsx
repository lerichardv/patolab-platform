...
interface SpecimenReport {
	...
	sections_order: Array<{ key: string; order: number; active: boolean }> | null;
}
...
function CollaborativeEditorInner({
	...
	return (
		<div className="space-y-1">
			...
			>
				{!isSynced && (
					<div className="absolute inset-0 z-50 flex flex-col items-center justify-center rounded-lg bg-background/85 backdrop-blur-xs select-none animate-in fade-in duration-200">
...
const isEmptyHtml = (html: string | null | undefined): boolean => {
	...
	if (typeof window === 'undefined') {
		...
		const cleanStr = html.replace(/<[^>]*>/g, '').replace(/\u00a0/g, ' ').trim();
...
export default function ReportWorkspace({
    ...
    const [sectionsOrder, setSectionsOrder] = useState<Array<{ key: string; order: number; active: boolean }>>(() => {
        if (report?.sections_order && Array.isArray(report.sections_order) && report.sections_order.length > 0) {
            return [...report.sections_order].sort((a, b) => a.order - b.order);
        }
	...
	const [saveStatusProvider, setSaveStatusProvider] =
		useState<HocuspocusProvider | null>(null);
	const [sectionsOrderDoc, setSectionsOrderDoc] = useState<Y.Doc | null>(null);
		...
		const handleOrderYjsChange = () => {
			...
			if (val) {
				try {
					...
					console.error("Failed to parse sections_order from Yjs update:", e);
				}
			}
		};
													...
													{(provided, snapshot) => (
														<div
															...
															className={cn(
																...
																snapshot.isDragging && "border-primary/20 bg-card/65 shadow-lg ring-1 ring-primary/10 backdrop-blur-xs rotate-1"
															)}
														>
															{isClin && (
																<>
																	<div
																		...
																		className="flex items-center gap-1.5 border-l-4 border-emerald-500/85 pl-2 cursor-grab active:cursor-grabbing hover:bg-slate-100/50 dark:hover:bg-slate-800/30 rounded-r-md py-0.5 transition-colors select-none"
																	>
															...
															{isDiag && (
																<>
																	<div
																		{...provided.dragHandleProps}
																		className="flex items-center gap-1.5 border-l-4 border-blue-500/80 pl-2 cursor-grab active:cursor-grabbing hover:bg-slate-100/50 dark:hover:bg-slate-800/30 rounded-r-md py-0.5 transition-colors select-none"
																	>
																		<GripVertical className="h-4 w-4 text-muted-foreground/60 shrink-0" />
																		<h3 className="flex items-center gap-2 text-base font-bold tracking-tight text-slate-800 dark:text-slate-200">
															...
															{isMacro && (
																<>
																	<div
																		{...provided.dragHandleProps}
																		className="flex items-center gap-1.5 border-l-4 border-violet-500/80 pl-2 cursor-grab active:cursor-grabbing hover:bg-slate-100/50 dark:hover:bg-slate-800/30 rounded-r-md py-0.5 transition-colors select-none"
																	>
																		<GripVertical className="h-4 w-4 text-muted-foreground/60 shrink-0" />
																		<h3 className="flex items-center gap-2 text-base font-bold tracking-tight text-slate-800 dark:text-slate-200">
															...
															{isMicro && (
																<>
																	<div
																		{...provided.dragHandleProps}
																		className="flex items-center gap-1.5 border-l-4 border-fuchsia-500/80 pl-2 cursor-grab active:cursor-grabbing hover:bg-slate-100/50 dark:hover:bg-slate-800/30 rounded-r-md py-0.5 transition-colors select-none"
																	>
																		<GripVertical className="h-4 w-4 text-muted-foreground/60 shrink-0" />
																		<h3 className="flex items-center gap-2 text-base font-bold tracking-tight text-slate-800 dark:text-slate-200">
																							...
																							<AlertDialog>
																								...
																								<AlertDialogContent className="max-w-4xl">
																									<AlertDialogHeader>
																										...
																										<AlertDialogDescription>
																											Antes de finalizar,
																											verifique el formato en
																											la vista previa del PDF
																											interactivo.
																										</AlertDialogDescription>
																									</AlertDialogHeader>
																									<div className="flex justify-center border bg-slate-50 p-4 dark:bg-slate-900 rounded-lg max-h-[60vh] overflow-y-auto">
																										<div
																											ref={
																									...
																									{!hasScrolledToEnd && (
																										<p className="text-center text-[11px] text-muted-foreground animate-pulse font-medium">
																											Desplace hasta el
																											final de la
																											previsualización
																											para habilitar la
															...
															{isComm && (
																<>
																	<div
																		{...provided.dragHandleProps}
																		className="flex items-center gap-1.5 border-l-4 border-amber-500/85 pl-2 cursor-grab active:cursor-grabbing hover:bg-slate-100/50 dark:hover:bg-slate-800/30 rounded-r-md py-0.5 transition-colors select-none"
																	>
																		<GripVertical className="h-4 w-4 text-muted-foreground/60 shrink-0" />
																		<h3 className="flex items-center gap-2 text-base font-bold tracking-tight text-slate-800 dark:text-slate-200">
															...
															{isProt && (
																<>
																	<div
																		{...provided.dragHandleProps}
																		className="flex items-center gap-1.5 border-l-4 border-blue-600/85 pl-2 cursor-grab active:cursor-grabbing hover:bg-slate-100/50 dark:hover:bg-slate-800/30 rounded-r-md py-0.5 transition-colors select-none"
																	>
																		<GripVertical className="h-4 w-4 text-muted-foreground/60 shrink-0" />
																		<h3 className="flex items-center gap-2 text-base font-bold tracking-tight text-slate-800 dark:text-slate-200">
															...
															{isLeg && (
																<>
																	<div
																		{...provided.dragHandleProps}
																		className="flex items-center gap-1.5 border-l-4 border-slate-500/85 pl-2 cursor-grab active:cursor-grabbing hover:bg-slate-100/50 dark:hover:bg-slate-800/30 rounded-r-md py-0.5 transition-colors select-none"
																	>
																		<GripVertical className="h-4 w-4 text-muted-foreground/60 shrink-0" />
																		<h3 className="flex items-center gap-2 text-base font-bold tracking-tight text-slate-800 dark:text-slate-200">
