...
export default function SpecimenGroupForm({
	group = null,
	open = true,
	onSuccess,
	onOpenChange,
	...
	// Manual delivery date overrides for nested specimen
	const [
		nestedIsManualDeliveryDateEnabled,
		setNestedIsManualDeliveryDateEnabled,
	] = useState(false);
	const [nestedDeliveryDateUnit, setNestedDeliveryDateUnit] = useState<
		'minutes' | 'hours' | 'days' | 'weeks'
	>('days');
	const [nestedDeliveryDateQuantity, setNestedDeliveryDateQuantity] =
		useState<number | string>('');
	const [
		nestedIsManualDeliveryDateInternEnabled,
		setNestedIsManualDeliveryDateInternEnabled,
	] = useState(false);
	const [nestedDeliveryDateInternUnit, setNestedDeliveryDateInternUnit] =
		useState<'minutes' | 'hours' | 'days' | 'weeks'>('days');
	const [
	...
	const handleOpenNestedForm = () => {
		...
		setNestedIsManualDeliveryDateEnabled(false);
		setNestedDeliveryDateUnit('days');
		setNestedDeliveryDateQuantity('');
		setNestedIsManualDeliveryDateInternEnabled(false);
		setNestedDeliveryDateInternUnit('days');
		setNestedDeliveryDateInternQuantity('');
	...
	const handleEditNestedSpecimen = (spec: any) => {
		...
		setNestedDeliveryDateInternQuantity(
			spec.delivery_date_intern_quantity != null
				? spec.delivery_date_intern_quantity
				: '',
		);
	...
	const handleSaveNestedSpecimen = async () => {
		...
		const specObject = {
			...
			delivery_date_intern_quantity:
				nestedIsManualDeliveryDateInternEnabled
					? parseInt(
						String(nestedDeliveryDateInternQuantity || '0'),
						10,
					)
					: 0,
	...
	const submitGroupForm = async () => {
		...
		const specimensData = specimens.map((s) => ({
			...
			delivery_date_unit: s.delivery_date_unit || 'days',
			delivery_date_quantity:
				parseInt(String(s.delivery_date_quantity || '0'), 10) || 0,
			is_manual_delivery_date_intern_enabled: Boolean(
				s.is_manual_delivery_date_intern_enabled,
			),
			delivery_date_intern_unit:
			...
			delivery_date_intern_quantity:
				parseInt(
					String(s.delivery_date_intern_quantity || '0'),
					10,
				) || 0,
		...
		<>
					...
					<div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center gap-4 bg-background/80 backdrop-blur-sm">
						...
						<div className="flex flex-col items-center text-center">
							...
							<p className="mt-1 max-w-xs text-sm text-muted-foreground">
								Estamos creando las muestras y compilando la
								factura grupal en PDF. Espere.
							</p>
						</div>
					</div>,
					document.body,
							...
							<div className="grid grid-cols-1 gap-4 pr-28 sm:grid-cols-3">
								...
								<div className="flex flex-col gap-1">
									...
									<span className="font-medium text-foreground">
										{selectedGlobalCustomer.phone || 'N/A'}
									</span>
								</div>
							</div>
						</div>
				...
				<div className="space-y-6 px-5">
					<div className="flex items-center justify-between border-b pb-4">
						...
						<Button
							...
							size="sm"
						>
							<Plus className="mr-1.5 h-4 w-4" /> Agregar Muestra
						</Button>
					</div>
						...
						<div className="space-y-3">
							...
							<div className="text-sky-850 flex items-start gap-2.5 rounded-lg border border-sky-100 bg-sky-50/50 px-3 py-2 text-xs dark:border-sky-950/40 dark:bg-sky-950/15 dark:text-sky-300">
								...
								<span>
									...
									Los códigos de secuencia para cada muestra
									agregada han sido reservados y se asignarán
									definitivamente al crear el grupo.
								</span>
							</div>
						</div>
					)}
			...
			{currentStep === 2 && (
				...
				>
					<div className="space-y-0.5 border-b pb-4">
						...
						<p className="text-xs text-muted-foreground">
							Configure el método de pago, cargue comprobantes y
							revise los insumos/precios de cada muestra.
						</p>
					</div>
								...
								<div className="flex items-center justify-between">
									<div className="flex flex-col gap-0.5">
										...
										<span className="text-[10px] text-muted-foreground">
											...
											para servicios adicionales del
											grupo.
										</span>
									</div>
									<Switch
							...
							<div className="flex flex-col gap-3 rounded-lg border bg-muted/30 p-4">
								...
								{paymentType ? (
									...
									<div className="mt-2 border-t pt-2.5 text-[11px] text-muted-foreground italic">
										Por favor, configure los detalles del
										pago.
									</div>
								)}
							</div>
			...
			<Sheet open={isNestedFormOpen} onOpenChange={setIsNestedFormOpen}>
				...
				>
					...
					<div className="space-y-5 px-5 py-4">
						...
						<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
							...
							<div className="grid gap-2">
								...
								{(() => {
									...
									const searchedExams = filteredExams.filter(
										(exam) => {
											if (!nestedExaminationSearchQuery) {
												return true;
											}
								...
								{matchingSequence ? (
									<div className="flex items-center gap-2 rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-3 text-emerald-800 dark:bg-emerald-500/10 dark:text-emerald-300">
										...
										<span className="text-xs font-semibold">
											...
											configurada para el tipo de muestra
										</span>
									</div>
								) : (
									<div className="flex items-start gap-2.5 rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 text-amber-800 dark:bg-amber-500/10 dark:text-amber-300">
										...
										<div className="flex flex-col gap-1">
											<span className="text-xs font-semibold text-amber-800 dark:text-amber-300">
												¡Falta secuencia de numeración!
											</span>
											<span className="text-[10.5px] leading-normal text-amber-600 dark:text-amber-400">
												No existe una secuencia activa
												para este tipo de muestra en la
												sucursal activa. Debe crear una
												antes de poder facturar.
											</span>
											<button
												type="button"
												onClick={() =>
													setIsSequenceSheetOpen(true)
												}
						...
						{/* Client Delivery Time Override */}
						<div className="space-y-3 rounded-lg border p-3">
							<div className="flex items-center justify-between">
								<div className="space-y-0.5">
									<Label
										htmlFor="nested-manual-delivery-client-toggle"
										className="cursor-pointer text-sm font-semibold"
									>
										Personalizar tiempo de entrega (Cliente)
									</Label>
									<p className="text-xs text-muted-foreground">
										Anula el tiempo de entrega definido por la
										categoría para el cliente.
									</p>
								</div>
								<Switch
									id="nested-manual-delivery-client-toggle"
									checked={nestedIsManualDeliveryDateEnabled}
									onCheckedChange={(checked) => {
								...
								<div className="space-y-4 border-t border-border/40 pt-2">
									<div className="grid grid-cols-2 gap-4">
										<div className="grid gap-2">
											...
											<Input
												id="nested_delivery_date_quantity"
												type="number"
												step="1"
												min="0"
												value={
										...
										<div className="grid gap-2">
											<Label htmlFor="nested_delivery_date_unit">
												Unidad
											</Label>
											<Select
						...
						{/* Internal Delivery Time Override */}
						<div className="space-y-3 rounded-lg border p-3">
							<div className="flex items-center justify-between">
								<div className="space-y-0.5">
									<Label
										htmlFor="nested-manual-delivery-intern-toggle"
										className="cursor-pointer text-sm font-semibold"
									>
										Personalizar tiempo de entrega (Interno)
									</Label>
									<p className="text-xs text-muted-foreground">
										Anula el tiempo límite de procesamiento
										interno definido por la categoría.
									</p>
								</div>
								<Switch
									id="nested-manual-delivery-intern-toggle"
									checked={
										nestedIsManualDeliveryDateInternEnabled
									}
									onCheckedChange={(checked) => {
								...
								<div className="space-y-4 border-t border-border/40 pt-2">
									<div className="grid grid-cols-2 gap-4">
										<div className="grid gap-2">
											...
											<Input
												id="nested_delivery_date_intern_quantity"
												type="number"
												step="1"
												min="0"
												value={
										...
										<div className="grid gap-2">
											<Label htmlFor="nested_delivery_date_intern_unit">
												Unidad
											</Label>
											<Select
												value={
						...
						<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
							<div className="grid gap-2">
								...
								<DatePicker
									...
									onChange={(v) => {
										setNestedSampleCollectionDate(v);
										setNestedErrors((prev) => ({
											...prev,
			...
			<PaymentMethodSheet
				...
				onSave={(paymentData: PaymentData) => {
					...
					setPaymentMethodDate(paymentData.payment_method_date);
					...
					setHasInitialPayment(paymentData.has_initial_payment);
					setInitialPaymentAmount(paymentData.initial_payment_amount);
					setInitialPaymentType(paymentData.initial_payment_type);
			...
			<CustomerSheet
				...
				onSuccess={(updatedCustomer: any) => {
					if (updatedCustomer) {
						if (customerSheetSource === 'global') {
							setSelectedGlobalCustomerData(updatedCustomer);
						} else if (customerSheetSource === 'nested') {
							setSelectedNestedCustomerData(updatedCustomer);
						} else if (customerSheetSource === 'table_row') {
							setSpecimens((prev) =>
								prev.map((s) => {
									if (
										s.client_id ===
			...
			<SpecimenGroupInvoiceSummaryAlertDialog
				open={showConfirm}
				onOpenChange={setShowConfirm}
				customerName={selectedGlobalCustomerData?.name}
				specimens={specimens}
				specimenTypes={specimenTypes}
				examinations={examinations}
				paymentTypeLabel={getPaymentTypeLabel(paymentType)}
				specimensBaseTotal={specimensBaseTotal}
				customAmountEnabled={customAmountEnabled}
				customAmountVal={customAmountVal}
				customAmountReason={customAmountReason}
				globalDiscountTotal={globalDiscountTotal}
				specimensAutoDiscount={specimensAutoDiscount}
				specimensAdditionalDiscount={specimensAdditionalDiscount}
				finalSubtotalVal={finalSubtotalVal}
				onConfirm={submitGroupForm}
			/>
		</>
	);
}
