...
export default function SpecimenForm({
	...
	const { data, setData, post, processing, errors, setError, clearErrors, isDirty } = useForm({
		...
		priority_id: specimen?.priority_id ? specimen.priority_id.toString() : (priorities && priorities.length > 0 ? priorities[0].id.toString() : ''),
	...
	React.useEffect(() => {
		if (data.specimen_type) {
			const hasValidExamSelected = filteredExaminations.some(e => e.id.toString() === data.specimen_type_examination);
			if (!hasValidExamSelected && data.specimen_type_examination !== '') {
	...
	const validateStep3 = () => {
		...
		if (data.additional_discount_enabled) {
			if (!data.additional_discount) {
				localErrors.additional_discount = 'El descuento adicional es requerido.';
			} else {
				const addDiscount = parseFloat(data.additional_discount);
				if (isNaN(addDiscount) || addDiscount < 0) {
					localErrors.additional_discount = 'El descuento adicional debe ser mayor o igual a 0.';
				} else if (addDiscount > (maxSpecimenPriceVal + customAmountVal - autoDiscountTotal)) {
		...
		const needsProof =
			...
			(data.payment_type === 'credit' && data.has_initial_payment && data.initial_payment_type !== 'cash');
	...
	const ageDiscountVal = React.useMemo(() => {
		...
		return 0;
	}, [data.selected_price, data.age_discount_type, thirdAgePercent, fourthAgePercent]);
	...
	React.useEffect(() => {
		const additionalDiscountVal = data.additional_discount_enabled ? (parseFloat(data.additional_discount) || 0) : 0;
	...
	}, [autoDiscountTotal, data.additional_discount_enabled, data.additional_discount]);
			...
			<form ref={formRef} onSubmit={handlePreSubmit} className="flex flex-col gap-6 py-4 px-5 pt-0">
						...
						<div className="flex items-center justify-between gap-5">
							<div className="flex items-center gap-2 w-full">
								...
								<div className='flex flex-col w-full gap-2'>
									...
									/>
									{errors.customer && <p className="text-sm text-destructive text-xs">{errors.customer}</p>}
								</div>
							</div>
							<button
								type="button"
						...
						{selectedCustomer && (
							<div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-3 border-t border-border/50 text-xs">
								<div className="flex flex-col gap-1">
									...
									<span className="font-mono text-foreground font-medium">{selectedCustomer.id_number || 'N/A'}</span>
								</div>
								<div className="flex flex-col gap-1">
									<span className="text-muted-foreground font-semibold uppercase tracking-wider text-[10px]">Correo Electrónico</span>
									<span className="text-foreground font-medium break-all">{selectedCustomer.email || 'Sin correo'}</span>
								</div>
								{selectedCustomer.type !== 'empresa' && (
									<div className="flex flex-col gap-1">
										<span className="text-muted-foreground font-semibold uppercase tracking-wider text-[10px]">Edad</span>
										<span className="text-foreground font-medium">{selectedCustomer.age ? `${selectedCustomer.age} años` : 'N/A'}</span>
									</div>
								)}
							</div>
						)}
				...
				{(!specimen ? currentStep === 1 : true) && (
					<div>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
							...
							<div className="grid gap-2">
								...
								<FormCombobox
									...
									options={filteredExaminations.map(e => ({ label: e.name, value: e.id.toString() }))}
						...
						<div className="grid gap-2 mt-4">
							<Label htmlFor="medical_order_file">Orden Médica (Archivo PDF o Imagen)</Label>
						...
						{!specimen && (
							<>
								<div className="flex items-center justify-between p-4 bg-muted/30 border rounded-lg mt-4">
									<div className="flex flex-col gap-0.5">
										<Label htmlFor="agregar-insumos-toggle" className="text-sm font-semibold cursor-pointer">
											Agregar insumos
										</Label>
										<span className="text-xs text-muted-foreground">
											Registre los insumos químicos o reactivos que se utilizarán en el análisis de esta muestra.
										</span>
									</div>
									<Switch
								...
								{data.agregar_insumos && (
									<div className="border rounded-xl p-4 bg-muted/10 border-border/60 mt-4 space-y-4">
										<div className="flex flex-col gap-1">
											<h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">
												Selección de Insumos / Reactivos
											</h3>
											<p className="text-xs text-muted-foreground">
												Busque y agregue los insumos químicos o reactivos que se utilizarán en el análisis de esta muestra.
											</p>
										</div>
															...
															<div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-sm text-muted-foreground gap-3">
																...
																<div className="flex flex-col gap-1 max-w-[200px]">
																	...
																	<span className="text-xs text-muted-foreground leading-normal">
																		Presione "Agregar" en la lista de insumos disponibles.
																	</span>
																</div>
															</div>
														) : (
							...
							<div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
								...
								<div className="lg:col-span-8 flex flex-col gap-5">
									...
									<div className="flex flex-col gap-3 p-4 bg-muted/30 border rounded-lg">
										<div className="flex items-center justify-between">
											<div className="flex flex-col gap-0.5">
												<Label htmlFor="additional-discount-toggle" className="text-xs font-semibold cursor-pointer">
													Descuento Adicional
												</Label>
												<span className="text-[10px] text-muted-foreground">
													Permite aplicar un descuento adicional personalizado a la factura.
												</span>
											</div>
											<Switch
												id="additional-discount-toggle"
												checked={data.additional_discount_enabled}
												onCheckedChange={(checked) => {
													setData(d => ({
														...d,
														additional_discount_enabled: checked,
														additional_discount: checked ? d.additional_discount || '0' : '0'
													}));
												}}
											/>
										</div>
										...
										{data.additional_discount_enabled && (
											<div className="flex flex-col gap-3 pt-2 border-t border-border/50 transition-all duration-300">
												<div className="grid gap-1.5">
													<Label htmlFor="additional_discount" className="text-xs">
														Monto de Descuento Adicional (L.) <span className="text-destructive">*</span>
													</Label>
													<Input
														...
														onChange={(e) => setData('additional_discount', e.target.value)}
														placeholder="0.00"
														required
													/>
													{errors.additional_discount && (
														<span className="text-[10px] text-destructive">{errors.additional_discount}</span>
													)}
												</div>
											</div>
										)}
									...
									<div className="flex flex-col gap-3 p-4 bg-muted/30 border rounded-lg">
										<div className="flex flex-col gap-0.5 border-b pb-2 mb-2">
											...
											<span className="text-[10px] text-muted-foreground">
												Aplique el descuento de la tercera o cuarta edad al paciente. Solo se puede aplicar uno a la vez.
											</span>
										</div>

										<div className="flex items-center justify-between">
											<div className="flex flex-col gap-0.5">
												<Label htmlFor="third-age-discount-toggle" className="text-xs font-semibold cursor-pointer">
													Tercera Edad ({thirdAgePercent}%)
												</Label>
												<span className="text-[10px] text-muted-foreground">
													Aplica {thirdAgePercent}% de descuento sobre el precio base.
												</span>
											</div>
											<Switch
												...
												onCheckedChange={(checked) => {
													setData('age_discount_type', checked ? 'third' : null);
												}}
											/>
										</div>
										...
										<div className="flex items-center justify-between border-t border-border/50 pt-3 mt-1">
											<div className="flex flex-col gap-0.5">
												<Label htmlFor="fourth-age-discount-toggle" className="text-xs font-semibold cursor-pointer">
													Cuarta Edad ({fourthAgePercent}%)
												</Label>
												<span className="text-[10px] text-muted-foreground">
													Aplica {fourthAgePercent}% de descuento sobre el precio base.
												</span>
											</div>
											<Switch
												...
												onCheckedChange={(checked) => {
													setData('age_discount_type', checked ? 'fourth' : null);
												}}
											/>
										</div>
									</div>
										...
										<div className="flex flex-col gap-3 mt-2">
											...
											{(autoDiscountTotal + additionalDiscountVal) > 0 ? (
												<div className="flex flex-col gap-1.5 bg-emerald-500/5 dark:bg-emerald-500/10 p-2.5 rounded-lg border border-emerald-500/20 text-emerald-800 dark:text-emerald-300">
													...
													{ageDiscountVal > 0 && (
														<div className="flex justify-between text-xs">
															<span>{data.age_discount_type === 'third' ? 'Tercera Edad' : 'Cuarta Edad'}:</span>
															<span className="font-semibold">- L. {ageDiscountVal.toFixed(2)}</span>
														</div>
													)}
													{additionalDiscountVal > 0 && (
														<div className="flex justify-between text-xs">
															<span>Descuento Adicional:</span>
															<span className="font-semibold">- L. {additionalDiscountVal.toFixed(2)}</span>
														</div>
													)}
													<Separator className="bg-emerald-500/20 my-1" />
													<div className="flex justify-between text-xs font-bold">
														<span>Descuento Total:</span>
														<span>- L. {(autoDiscountTotal + additionalDiscountVal).toFixed(2)}</span>
													</div>
												</div>
											) : (
												<div className="flex justify-between text-sm">
				...
				<div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t mt-4">
					...
					{specimen ? (
						...
						<Button key="btn-submit" type="submit" disabled={processing} className="w-full sm:w-auto">
							{processing && <Spinner className="mr-2" />}
			...
			<AlertDialog open={showConfirm} onOpenChange={setShowConfirm}>
				<AlertDialogContent className="max-w-[500px]">
					...
					<div className="grid gap-3 py-3 text-sm">
						...
						{data.agregar_insumos && data.insumos && data.insumos.length > 0 && (
							<div className="border-b pb-2 flex flex-col gap-1.5">
								<span className="font-medium text-muted-foreground text-[10px] uppercase tracking-wider">Insumos Seleccionados (Consumo Interno):</span>
									...
									{data.insumos.map((insumo: any) => (
										<div key={insumo.id} className="flex justify-between items-center text-xs bg-muted/30 p-1.5 rounded border border-border/50">
											...
											<div className="text-right shrink-0 font-mono text-xs">
												<span className="text-muted-foreground text-[10px] mr-2">{insumo.quantity}x</span>
												<span className="font-bold text-emerald-600">L. 0.00</span>
											</div>
										</div>
									))}
								</div>
						...
						{data.agregar_insumos && data.insumos && data.insumos.length > 0 && (
							<div className="flex justify-between border-b pb-2">
								...
								<span className="font-semibold text-emerald-600">L. 0.00 (Consumo Interno)</span>
							</div>
						)}
						{(autoDiscountTotal + additionalDiscountVal) > 0 ? (
