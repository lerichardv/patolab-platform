...
export default function SpecimenViewSheet({
	...
	const durationText =
		specimen.category?.intern_quantity && specimen.category?.intern_unit
			? `Int: ${specimen.category.intern_quantity} ${specimen.category.intern_unit === 'minutes'
				? 'minutos'
	...
	const clientDurationText =
		specimen.category?.quantity && specimen.category?.unit
			? `Cli: ${specimen.category.quantity} ${specimen.category.unit === 'minutes'
				? 'minutos'
	...
	return (
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent
				className="w-full overflow-y-auto sm:max-w-[90vw] md:max-w-[1100px] lg:max-w-[1200px]"
					...
					<div className="grid grid-cols-1 gap-6 px-5 md:grid-cols-2">
						...
						<div className="space-y-6">
														...
														>
															...
															<td className="p-3">
																<div className="flex flex-wrap gap-1">
																	{user.pivot?.macroscopy_access ? (
																		<Badge variant="outline" className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 text-[10px] font-semibold py-0">Macro</Badge>
																	) : (
																		<Badge variant="outline" className="bg-muted text-muted-foreground border-border text-[10px] font-semibold py-0">No Macro</Badge>
																	)}
																	{user.pivot?.microscopy_access ? (
																		<Badge variant="outline" className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 text-[10px] font-semibold py-0">Micro</Badge>
																	) : (
																		<Badge variant="outline" className="bg-muted text-muted-foreground border-border text-[10px] font-semibold py-0">No Micro</Badge>
																	)}
																</div>
															</td>
														</tr>
							...
							<div className="space-y-4 rounded-lg border bg-card p-5 text-card-foreground shadow-sm">
								<div className="flex items-center justify-between">
									<h3 className="flex items-center gap-2 text-lg font-semibold text-primary">
										<UserPlus className="h-5 w-5" /> Colaboradores
										Asignados
									</h3>
									{onAssignPathologistClick &&
										(auth.permissions?.includes('specimens.manage') ||
											specimen.users?.some((u: any) => u.id === auth.user?.id)) && (
											<Button
												variant="ghost"
												size="sm"
												onClick={() => {
								...
								{specimen.collaborators && specimen.collaborators.length > 0 ? (
									...
									<div className="space-y-3 rounded-lg border border-dashed bg-muted/10 px-4 py-6 text-center">
										<p className="text-xs text-muted-foreground">
											No hay colaboradores asignados a esta
											muestra.
										</p>
										{onAssignPathologistClick &&
											(auth.permissions?.includes('specimens.manage') ||
