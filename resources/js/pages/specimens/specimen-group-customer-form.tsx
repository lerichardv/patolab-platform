...
export default function SpecimenGroupCustomerForm({
	...
	const computedNewGroupName = useMemo(() => {
		...
		const targetName = selectedCustomer?.name || 'Nuevo Cliente';
	...
	const handleSubmit = (e: React.FormEvent) => {
		e.preventDefault();
		if (!selectedCustomer || !isCustomerChanged) {
			return;
		}
		...
		router.put(
			...
			},
			{
				...
				onSuccess: () => {
					...
					setIsDirty?.(false);
					toast.success(
						'Cliente principal del grupo actualizado con éxito.',
					);
					onSuccess?.();
				},
				onError: (errors) => {
		...
		<form onSubmit={handleSubmit} className="space-y-6 py-4 px-5">
			...
			<div className="space-y-3">
				<div className="flex items-center justify-between">
					<label className="text-xs font-bold tracking-wider text-muted-foreground uppercase">
						Nuevo Cliente Principal <span className="text-destructive">*</span>
					</label>
					{isCustomerChanged && (
						<Badge
							variant="secondary"
				...
				<AsyncCustomerCombobox
					placeholder="Buscar nuevo cliente por nombre o RTN..."
					value={selectedCustomerId}
					initialCustomer={groupInfo.customer}
					onChange={handleCustomerChange}
				/>

				{/* Selected New Customer Card Preview */}
				{selectedCustomer && isCustomerChanged && (
			...
			<div className="flex items-center justify-end gap-2 pt-2">
				...
				<Button
					type="submit"
					disabled={!isCustomerChanged || submitting}
