...
export default function ManageCuttingsSheet({
	...
	const [loadingCuttingId, setLoadingCuttingId] = useState<number | null>(null);
	...
	const handleStatusChange = (
		...
		router.put(
			...
			{ status: nextStatus },
			{
				onSuccess: () => {
					toast.success(
						`Estado del corte "${cutting.description}" cambiado a ${nextStatus === 'processing'
							? 'Procesamiento'
														...
														{c.cutting_slide_types &&
															...
															<div className="flex flex-wrap gap-1">
																{c.cutting_slide_types.map(
																	(stId, i) => {
																		const typeName = cuttingSlideTypes.find(
																			(t) => String(t.id) === String(stId)
																		)?.name || `ID: ${stId}`;
														...
														<div className="flex items-center justify-end gap-1.5 opacity-80 transition-opacity group-hover:opacity-100">
															{loadingCuttingId === c.id ? (
																...
																>
																	...
																	<SelectContent>
																		<SelectItem value="macroscopy" className="text-xs">
																			Macroscopía
																		</SelectItem>
