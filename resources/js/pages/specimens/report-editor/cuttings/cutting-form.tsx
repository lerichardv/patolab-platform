...
export default function CuttingForm({
	...
	const [isCreateWorkOrderTypeOpen, setIsCreateWorkOrderTypeOpen] = useState(false);
	...
	useEffect(() => {
		if (cuttingSlideTypes.length > prevSlideTypesRef.current.length) {
			const newTypes = cuttingSlideTypes.filter(
				(st) => !prevSlideTypesRef.current.some((prev) => prev.id === st.id),
			);
	...
	useEffect(() => {
		if (cutting) {
			setData({
				...
				cuttings_description: cutting.cuttings_description || '',
				number_of_slides: cutting.number_of_slides ?? 1,
				cutting_slide_types: (cutting.cutting_slide_types || []).map(String),
				comments: cutting.comments || '',
		...
		<>
			<form onSubmit={submit} className="space-y-5 px-5 py-4">
				{isDuplicate && (
					<div className="flex gap-3 rounded-lg border border-blue-200 bg-blue-50/50 p-3.5 text-xs text-blue-800 dark:border-blue-900/50 dark:bg-blue-950/20 dark:text-blue-300">
						...
						<div>
							...
							Se ha copiado la información del corte original. Realice las modificaciones necesarias para registrar este nuevo corte.
						</div>
					</div>
				)}
				...
				<div className="space-y-2">
					<div className="flex items-center justify-between">
						<Label htmlFor="cutting_slide_types">
							Tipo Láminas T. Especial <small>(Tipo Orden de Trabajo)</small>
						</Label>
						<button
							type="button"
				...
				<div className="flex justify-end pt-3">
					...
					>
						{isEditMode ? 'Actualizar Corte' : isDuplicate ? 'Duplicar Corte' : 'Registrar Corte'}
					</Button>
				</div>
			</form>
