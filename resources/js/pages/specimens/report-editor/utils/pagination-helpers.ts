...
export function parseColumnPercentages(tableHtml: string, colCount: number): number[] {
	...
	if (colgroupMatch) {
		...
		while ((colMatch = colRegex.exec(colgroupHtml)) !== null) {
			...
			const styleWidthMatch = colHtml.match(/style=["'][^"']*width:\s*([\d.]+)(px|%)?[^"']*["']/i);
			if (styleWidthMatch) {
				width = parseFloat(styleWidthMatch[1]);
			} else {
				const widthAttrMatch = colHtml.match(/width=["']([\d.]+)%?["']/i);
				if (widthAttrMatch) {
	...
	const explicitWidths = finalColWidths.filter(w => w !== null && w > 0) as number[];
	const avgWidth = explicitWidths.reduce((a, b) => a + b, 0) / explicitWidths.length;
	
	const filledWidths = finalColWidths.map(w => (w !== null && w > 0) ? w : avgWidth);
	const totalWidth = filledWidths.reduce((a, b) => a + b, 0);
	
	return totalWidth > 0 ? filledWidths.map(w => (w / totalWidth) * 100.0) : Array(colCount).fill(100.0 / colCount);
}
	...
	for (const trHtml of allMatches) {
		...
		while ((cellMatch = cellRegex.exec(trHtml)) !== null) {
			...
			const decodedInner = decodeHtmlEntities(cellInner)
				...
				.replace(/\u2007/g, ' ')
				.replace(/\u202F/g, ' ');
			const isListCell = /<(?:ul|ol|li)[^>]*>/i.test(decodedInner) || /[–-]\s{2,}/.test(decodedInner);
