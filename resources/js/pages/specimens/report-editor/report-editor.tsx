...
const EditorRegistryContext = React.createContext<{
...
export default function ReportWorkspace({
	...
	>(() => {
		if (
			...
			const loaded = [...report.sections_order].sort((a, b) => a.order - b.order);
	...
	const calculateLayout = () => {
			...
			for (let bIndex = 0; bIndex < blocksList.length; bIndex++) {
				...
				if (block.type === 'image-grid') {
					...
					while (rowIndex < rowsRemaining.length) {
						...
						if (remaining < minGridHeight && currentPageList.length > 0) {
							pagesList.push(currentPageList);
							currentPageList = [];
							currentHeightList = 0.0;
							continue;
						...
						let r = 0;
						for (let tempR = 1; tempR <= rowsRemaining.length - rowIndex; tempR++) {
							let cost = 2.0;
						...
						for (let i = 0; i < r; i++) {
							rowsRemaining[rowIndex + i].forEach((imgTag) => {
								...
								const styleMatch = processedTag.match(/style=["\']([^"\']*)["\']/i);
								if (styleMatch) {
									const existingStyle = styleMatch[1].replace(/;$/, '');
									const newStyle = `${existingStyle}; aspect-ratio: 1 / ${aspect}; object-fit: cover;`;
									processedTag = processedTag.replace(/style=["\']([^"\']*)["\']/i, `style="${newStyle}"`);
								} else {
									processedTag = processedTag.replace('<img', `<img style="aspect-ratio: 1 / ${aspect}; object-fit: cover;"`);
								}
				...
				if (block.type === 'paragraph') {
					...
					const lines = splitHtmlIntoLines(paraInnerHtml, maxCharsPerLine);
					...
					while (i < lines.length) {
						...
						const style = isLastSlice ? '' : 'style="margin-bottom: 0px;"';
				...
				if (block.type === 'list') {
					...
					while (i < listItems.length) {
						...
						const itemPlainText = itemHtml.replace(/<[^>]+>/g, '').trim();
						...
						if (itemHeight > remaining) {
							if (currentHeightList === 0.0) {
								...
								currentHeightList += itemHeight + 0.5 * fontLineHeight;
								i++;
								olStartIndex++;
							} else {
								pagesList.push(currentPageList);
							...
							while (i < listItems.length) {
								...
								const nextItemLines = Math.max(
									1,
									Math.ceil(
										nextItemPlainText.length / (maxCharsPerLine - 5),
									),
								);
								const nextItemHeight = nextItemLines * fontLineHeight;
				...
				if (block.type === 'table') {
					...
					while (i < rows.length) {
						...
						const headerHeight = headerHtml ? 2.0 * fontLineHeight : 0.0;
						...
						while (i < rows.length) {
							const row = rows[i];
							const charsPerCell = Math.floor(maxCharsPerLine / colCount);
							const rowLines =
								Math.max(
									1,
							...
							const tableSpacing = isLastRow ? 1.0 * fontLineHeight : 0.0;
		...
		if (!isEmptyHtml(addendumHtmlValue)) {
			...
			const showAddendumHeading = headingsToggles['addendum_html'] ?? true;
	...
	const [openTextProvider, setOpenTextProvider] =
		useState<HocuspocusProvider | null>(null);
	const [openTextLabelDoc, setOpenTextLabelDoc] = useState<Y.Doc | null>(null);
	...
	const [addendumProvider, setAddendumProvider] =
		useState<HocuspocusProvider | null>(null);
	const [headingsTogglesDoc, setHeadingsTogglesDoc] = useState<Y.Doc | null>(null);
		...
		fetch(
			...
			.then((data) => {
				...
				if (headingsTogglesDoc) {
					...
					ytextToggles.insert(0, JSON.stringify(template.headings_toggles || {}));
				}
			...
			if (val) {
				try {
					...
					if (Array.isArray(parsed) && parsed.length > 0) {
						...
						const parsedKeys = new Set(parsed.map((s: { key: string }) => s.key));
			...
			>
				...
				<div className="h-[100vh] items-start bg-slate-50/50 dark:bg-slate-900/10">
					...
					<div className="h-[calc(100vh-64px)] w-screen overflow-auto lg:w-[50vw]">
						...
						<div className="flex flex-col gap-5 p-6">
								...
								{specimen.products &&
									...
									<div className="flex flex-col items-center justify-center gap-2 rounded-lg border border-dashed p-6 text-center text-xs text-muted-foreground">
										...
										<span className="font-medium">
											No se han registrado insumos o
											reactivos para esta muestra.
										</span>
										{(!isFinished ||
							...
							{/* Report Date Picker Card and Template Selector */}
							<div className="flex w-full flex-col items-start gap-4 rounded-xl border border-border bg-card/50 p-4 shadow-xs md:flex-row md:items-end md:justify-start">
								<div className="flex flex-col items-start gap-2">
									<label
										htmlFor="report-date"
										className="flex items-center gap-2 text-sm font-semibold whitespace-nowrap text-foreground"
									>
									...
									<div className="relative max-w-sm">
										<input
											...
											className="h-9 w-full rounded-md border border-input bg-card px-3 py-2 text-sm shadow-xs focus-visible:ring-1 focus-visible:ring-ring focus-visible:outline-hidden disabled:cursor-not-allowed disabled:opacity-60"
										/>
									</div>
								</div>
								...
								{templates && templates.length > 0 && (
									<div className="flex flex-col items-start gap-2">
										<span className="flex items-center gap-2 text-sm font-semibold whitespace-nowrap text-foreground">
										...
										<div className="w-full sm:w-[280px]">
											...
											>
												<SelectTrigger className="h-9 w-full bg-card text-sm text-foreground">
													<SelectValue placeholder="Seleccione una plantilla..." />
												</SelectTrigger>
												<SelectContent className="max-h-[300px]">
													{templates.map((temp) => (
									...
									<>
										{editorTemplateId && !isFinished && (
											<Button
												...
												className="flex h-9 items-center gap-1.5 bg-primary px-6 text-sm text-primary-foreground hover:bg-primary/95"
											>
												<Check className="h-4 w-4" />
												Aplicar
											</Button>
																		...
																		<>
																			...
																			>
																				<div className="flex items-center gap-1.5 w-full mr-4">
																					...
																					{(!['finalized', 'delivered'].includes(specimen.status) || (isFinished && sessionEditingEnabled)) && (hasMacroAccess || hasMicroAccess) ? (
																						<input
																							type="text"
																							value={openTextLabel}
																							onChange={(e) => handleOpenTextLabelChange(e.target.value)}
																							className="w-full bg-transparent border-b border-transparent hover:border-slate-300 focus:border-primary px-1 py-0.5 text-base font-bold tracking-tight text-slate-800 dark:text-slate-200 focus:outline-hidden"
				...
				>
					<AlertDialogContent className="max-w-4xl">
						<AlertDialogHeader>
							<AlertDialogTitle>
								¿Confirmar completado de microscopía y
								previsualizar reporte?
							</AlertDialogTitle>
							<AlertDialogDescription>
								Antes de finalizar, verifique el formato en la
								vista previa del PDF real.
							</AlertDialogDescription>
						</AlertDialogHeader>
						...
						{!hasScrolledToEnd && (
							<p className="mt-2 animate-pulse text-center text-[11px] font-medium text-muted-foreground">
								Desplace hasta el final de la previsualización
								para habilitar la finalización del reporte.
							</p>
						)}
