...
export default function UserCommissionsIndex({ commissions, users, specimenTypes, filters }: Props) {
	...
	const renderSortHeader = (field: string, label: string, isRightAlign: boolean = false) => {
		...
		return (
			<button
				...
				className={`group/btn inline-flex items-center gap-1.5 font-semibold transition-colors hover:text-foreground ${
					isRightAlign ? 'ml-auto justify-end text-right' : 'text-left'
				}`}
			>
				<span>{label}</span>
				{direction === 'asc' ? (
		...
		<>
			...
			<div className="flex h-full flex-1 flex-col gap-4 p-4">
				...
				<div className="flex w-full flex-col gap-4">
					...
					<div className="flex flex-col gap-4 sm:flex-row sm:items-end justify-stretch">
						...
						<TableHeader>
							<TableRow>
								...
								<TableHead className="text-right">
									{renderSortHeader('base_amount', 'Base Factura', true)}
								</TableHead>
								<TableHead className="text-right">
									{renderSortHeader('amount', 'Comisión', true)}
								</TableHead>
												...
												<div className="flex flex-col gap-1">
													<div className="flex items-center gap-1.5">
														<span className="w-max rounded border border-primary/20 bg-primary/5 px-1.5 py-0.5 font-mono text-[10px] font-semibold text-primary dark:bg-primary/10">
															{commission.specimen.sequence_code}
														</span>
														<button
															className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-[color,box-shadow] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive hover:text-accent-foreground size-9 h-5 w-5 hover:bg-muted"
															title="Ver Muestra"
															onClick={() => {
																setViewSpecimen(commission.specimen);
																setIsSpecimenViewOpen(true);
															}}
														>
														...
														<button
															...
															onClick={() => window.open(`/specimens?specimen=${commission.specimen?.sequence_code}&action=edit`, '_blank')}
														>
															<SquarePen className="h-3.5 w-3.5 text-muted-foreground hover:text-foreground" />
														</button>
													</div>
													<span className="text-[10px] text-muted-foreground" title={commission.specimen.type?.name || 'N/A'}>
														{commission.specimen.type?.name || 'N/A'} - {commission.specimen.examination?.name || 'N/A'}
													</span>
												</div>
											) : (
						...
						<div className="flex h-full flex-col gap-6 pb-8">
							...
							<div className="space-y-6 px-5">
									...
									<div className="grid grid-cols-2 gap-4 text-xs sm:grid-cols-2">
										<div>
											...
											<span className="font-semibold text-foreground">
												{detailsCommission.user?.name || 'N/A'}
											</span>
										</div>
										<div>
											<span className="block text-[10px] font-bold tracking-wider text-muted-foreground uppercase">
											...
											{detailsCommission.specimen ? (
												...
												>
													{detailsCommission.specimen.sequence_code}{' '}
													<ExternalLink className="h-3 w-3" />
												</a>
											) : (
												<span className="text-muted-foreground italic">Muestra eliminada</span>
											)}
										</div>
										<div>
											<span className="block text-[10px] font-bold tracking-wider text-muted-foreground uppercase">
											...
											<div className="mt-1">
												{detailsCommission.phase === 'macroscopy' ? (
													<Badge
														variant="outline"
										...
										<div>
											...
											<span className="font-semibold text-foreground">
												{detailsCommission.created_at ? formatDate(detailsCommission.created_at) : formatDate(detailsCommission.paid_at)}
											</span>
										</div>
									</div>

									{(detailsCommission.specimen?.type || detailsCommission.specimen?.examination) && (
										<>
											<Separator className="my-2" />
											<div className="space-y-1.5 text-xs">
												{detailsCommission.specimen?.type && (
													<div>
														<span className="font-semibold text-muted-foreground">Tipo de Muestra:</span>{' '}
														<span className="text-foreground">{detailsCommission.specimen.type.name}</span>
													</div>
												)}
												{detailsCommission.specimen?.examination && (
													<div>
														<span className="font-semibold text-muted-foreground">Examen:</span>{' '}
														<span className="text-foreground">{detailsCommission.specimen.examination.name}</span>
													</div>
												)}
											</div>
										</>
									...
									<div className="flex items-center justify-between">
										<div className="space-y-1">
											...
											<div className="mt-0.5">
												...
												) : detailsCommission.status === 'cancelled' ? (
													<Badge
														variant="outline"
										...
										{detailsCommission.status === 'paid' && detailsCommission.paid_at && (
											<div className="text-right text-xs">
												<span className="block text-[10px] font-bold tracking-wider text-muted-foreground uppercase">
													Fecha de Pago
												</span>
												<span className="font-semibold text-foreground">
													{formatDate(detailsCommission.paid_at)}
												</span>
												{detailsCommission.paid_by && (
													<span className="block text-[10px] text-muted-foreground mt-0.5">
														por {detailsCommission.paid_by.name}
													</span>
												)}
											</div>
										)}
								...
								<div className="space-y-4 rounded-lg border bg-card p-4 shadow-xs">
									...
									<div className="space-y-3 text-xs">
										<div className="flex justify-between">
											<span className="text-muted-foreground">Base Factura de Muestra:</span>
											<span className="font-mono font-semibold">
												{formatCurrency(detailsCommission.specimen_base_amount)}
											</span>
										</div>

										{(() => {
											const ruleApplied = detailsCommission.user_commission_rule_applied || {};
											const phase = detailsCommission.phase;
											const calcType = ruleApplied[`${phase}_calculation_type`] || 'N/A';
											const value = ruleApplied[`${phase}_commission_value`] || '0';

											return (
												<>
													<div className="flex justify-between">
														<span className="text-muted-foreground">Tipo de Regla Aplicada:</span>
														<span className="font-semibold capitalize">
															{calcType === 'fixed' ? 'Monto Fijo' : calcType === 'percentage' ? 'Porcentaje' : calcType}
														</span>
													</div>
													<div className="flex justify-between">
														<span className="text-muted-foreground">Valor de la Regla:</span>
														<span className="font-semibold">
															{calcType === 'fixed' ? formatCurrency(String(value)) : `${value}%`}
														</span>
													</div>

													<div className="mt-4 rounded-md bg-muted/50 p-3.5 border border-muted-foreground/10">
														<span className="block text-[10px] font-bold tracking-wider text-muted-foreground uppercase mb-1.5">
															Fórmula de Cálculo
														</span>
															...
															{calcType === 'percentage' ? (
																<>
																	<div className="flex justify-between text-muted-foreground">
																		<span>Base Muestra:</span>
																		<span>{formatCurrency(detailsCommission.specimen_base_amount)}</span>
																	</div>
																	<div className="flex justify-between text-muted-foreground">
																	...
																	<div className="flex justify-between font-bold text-emerald-600 dark:text-emerald-400">
																		...
																		<span>{formatCurrency(detailsCommission.calculated_comission_amount)}</span>
																	</div>
																</>
															) : (
																<>
																	<div className="flex justify-between text-muted-foreground">
																		<span>Monto Fijo de Regla:</span>
																		<span>{formatCurrency(String(value))}</span>
																	</div>
																	<Separator className="my-1" />
																	<div className="flex justify-between font-bold text-emerald-600 dark:text-emerald-400">
																		<span>Comisión Calculada:</span>
								...
								{detailsCommission.user_commission_rule_applied && (
									<div className="space-y-3 rounded-lg border bg-card p-4 shadow-xs">
										<h3 className="flex items-center gap-2 text-xs font-bold tracking-wider text-muted-foreground uppercase">
											Regla de Comisión Utilizada (Histórica)
										</h3>
										<Separator />
										<div className="grid grid-cols-2 gap-3 text-xs">
											<div>
												...
												<span className="font-mono text-foreground">
													#{detailsCommission.user_commission_rule_applied.id || detailsCommission.user_commission_rule_id}
												</span>
											</div>
											<div>
												<span className="block text-[10px] font-bold tracking-wider text-muted-foreground uppercase">
												...
												<span className="text-foreground font-semibold">
													{detailsCommission.rule ? (
														<span className="text-emerald-600 dark:text-emerald-400">Activa en el sistema</span>
													) : (
														<span className="text-rose-500 font-semibold italic">Eliminada (Histórica)</span>
													)}
												</span>
											</div>
											<div className="col-span-2 mt-1 space-y-2 rounded-md bg-muted/30 p-2.5">
												<div>
													...
													<span className="text-xs">
														{detailsCommission.user_commission_rule_applied.macroscopy_commission_enabled ? (
															<span>
																Habilitado ({detailsCommission.user_commission_rule_applied.macroscopy_calculation_type === 'fixed' ? 'Fijo' : 'Porcentaje'}:{' '}
																<strong>
																	{detailsCommission.user_commission_rule_applied.macroscopy_calculation_type === 'fixed'
																		? formatCurrency(String(detailsCommission.user_commission_rule_applied.macroscopy_commission_value))
																		: `${detailsCommission.user_commission_rule_applied.macroscopy_commission_value}%`}
																</strong>
																)
															</span>
														) : (
															<span className="text-muted-foreground italic">Deshabilitado</span>
														)}
													</span>
												</div>
												<div>
													...
													<span className="text-xs">
														{detailsCommission.user_commission_rule_applied.microscopy_commission_enabled ? (
															<span>
																Habilitado ({detailsCommission.user_commission_rule_applied.microscopy_calculation_type === 'fixed' ? 'Fijo' : 'Porcentaje'}:{' '}
																<strong>
																	{detailsCommission.user_commission_rule_applied.microscopy_commission_value !== undefined
																		? detailsCommission.user_commission_rule_applied.microscopy_calculation_type === 'fixed'
																			? formatCurrency(String(detailsCommission.user_commission_rule_applied.microscopy_commission_value))
								...
								<div className="space-y-2">
									<div className="flex items-center justify-between">
										<h3 className="text-xs font-bold tracking-wider text-muted-foreground uppercase">
											Registro de Snapshot JSON (Histórico)
										</h3>
										<Button
											variant="ghost"
											size="sm"
											className="h-7 px-2 text-[10px] gap-1 hover:bg-muted"
											onClick={() => copyToClipboard(JSON.stringify(detailsCommission.user_commission_rule_applied, null, 4))}
										>
											{copied ? (
												<>
									...
									<div className="relative rounded-md border bg-slate-950 p-4 font-mono text-[11px] leading-relaxed text-slate-100 dark:bg-slate-900/60 dark:border-slate-800">
										<pre className="max-h-[200px] overflow-auto scrollbar-thin scrollbar-thumb-muted-foreground/20">
											<code>
												{JSON.stringify(detailsCommission.user_commission_rule_applied, null, 4)}
											</code>
										</pre>
									</div>
									<p className="text-[10px] text-muted-foreground italic">
										* Este JSON es de solo lectura y contiene los valores exactos aplicados en la regla de comisión en la fecha de finalización.
									</p>
								</div>
							</div>
						</div>
			...
			<SpecimenViewSheet
				...
				onEditClick={() => {
					...
					if (viewSpecimen) {
						window.open(`/specimens?specimen=${viewSpecimen.sequence_code}&action=edit`, '_blank');
					}
				}}
			/>
		</>
