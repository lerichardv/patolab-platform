...
interface BillingReportRow {
	...
	examinations?: (ExaminationItem | string)[];
...
function getExamInfo(exam: ExaminationItem | string): ExaminationItem {
...
function ServiceCell({ row }: { row: BillingReportRow }) {
	const exams = row.examinations || [];
			...
			<div className="flex min-w-0 flex-col gap-0.5">
				{row.specimen_type ? (
					<>
						...
						{exams.length > 1 ? (
							<ul className="space-y-1 pl-0.5 text-[11px] text-muted-foreground">
								{exams.map((exam, idx) => {
									const item = getExamInfo(exam);
							...
							(() => {
								const item = getExamInfo(exams[0]);
