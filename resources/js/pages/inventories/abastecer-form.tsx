			...
			<div className="space-y-2">
				...
				<Select
					...
					onValueChange={(value) => {
						...
						if (inv && !data.items.some(item => item.inventory_id === inventoryId)) {
							setData('items', [
								...data.items,
				...
				>
					...
					<SelectContent>
						{inventories.map(item => {
							...
							return (
								<SelectItem key={item.id} value={item.id.toString()} disabled={disabled}>
									{item.product_relation.name} ({item.product_relation.code}) - {item.storage_relation.name} (Actual: {item.quantity}) {disabled && '(Agregado)'}
								</SelectItem>
							);
						})}
					</SelectContent>
							...
							{data.items.length > 0 ? (
										...
										<TableRow key={item.inventory_id}>
											<TableCell className="py-2">
												<div className="flex flex-col">
													<span className="font-medium text-sm">{inv?.product_relation.name}</span>
													<span className="text-[10px] text-muted-foreground">
														Bodega: {inv?.storage_relation.name} | Actual: {inv?.quantity}
													</span>
												</div>
											</TableCell>
											<TableCell className="py-2">
								...
								<TableRow>
									<TableCell colSpan={3} className="h-24 text-center text-muted-foreground">
										No hay productos agregados para abastecer. Seleccione uno arriba.
									</TableCell>
								</TableRow>
							)}
						</TableBody>
