...
export default function InventoryForm({ inventory, storages, products, existingInventories, onSuccess }: Props) {
	const { data, setData, post, put, processing, errors, reset } = useForm<{
		...
		storage: inventory?.storage?.toString() || (storages.length === 1 ? storages[0].id.toString() : ''),
	...
	const isProductDisabled = (productId: number) => {
		...
		const existsInBackend = existingInventories.some(
			(inv) => inv.storage === selectedStorageId && inv.product === productId
		);
		if (existsInBackend) return true;

		if (!inventory?.id) {
			const existsInTable = data.items.some(item => item.product === productId);
			if (existsInTable) return true;
		}
	...
	const getProductDisabledReason = (productId: number) => {
		...
		if (existsInBackend) return '(Ya en bodega)';

		if (!inventory?.id) {
			const existsInTable = data.items.some(item => item.product === productId);
			if (existsInTable) return '(Agregado)';
		}
	...
	const handleStorageChange = (storageIdStr: string) => {
		...
		const filteredItems = data.items.filter(item => {
			const exists = existingInventories.some(
				inv => inv.storage === newStorageId && inv.product === item.product
			);
			return !exists;
		});
	...
	return (
		<form onSubmit={submit} className="space-y-6 py-6 px-5">
			...
			<div className="space-y-2">
				...
				<Select
					...
					onValueChange={(value) => {
						if (inventory?.id) {
							...
							if (prod && !data.items.some(item => item.product === productId)) {
								setData('items', [
									...data.items,
				...
				>
					<SelectTrigger className="w-full">
						<SelectValue placeholder={!data.storage ? "Seleccione primero un almacén" : "Seleccione un producto"} />
					</SelectTrigger>
					<SelectContent>
						{products.map(product => {
							...
							return (
								<SelectItem
									...
									disabled={!!reason}
								>
									{product.name} ({product.code}) {reason && ` ${reason}`}
								</SelectItem>
							);
						})}
					</SelectContent>
								...
								{data.items.length > 0 ? (
									...
									<TableRow>
										<TableCell colSpan={3} className="h-24 text-center text-muted-foreground">
											No hay productos agregados. Seleccione uno arriba.
										</TableCell>
									</TableRow>
								)}
							</TableBody>
