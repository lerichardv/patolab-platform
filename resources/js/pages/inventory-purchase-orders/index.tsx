...
export default function PurchaseOrdersIndex({
	...
	const [orderToRecibir, setOrderToRecibir] = useState<PurchaseOrder | null>(null);
	const [isConfirmReabrirOpen, setIsConfirmReabrirOpen] = useState(false);
	const [orderToReabrir, setOrderToReabrir] = useState<PurchaseOrder | null>(null);
	...
	return (
		<>
				...
				<div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
					<div>
						...
						<p className="text-muted-foreground">
							Administre las órdenes de compra y el abastecimiento de insumos.
						</p>
					</div>
											...
											<TableCell className="font-mono font-semibold uppercase">
												...
												{order.purchase_order_file && (
													<Button
														...
														size="sm"
														onClick={() => {
															setPdfPreviewUrl(`/storage/${order.purchase_order_file}`);
															setShowPdfModal(true);
														}}
														className="mt-1.5 h-6 rounded-lg px-2.5 text-[10px] font-medium cursor-pointer"
													>
														<ExternalLink className="h-2 w-2" /> Ver PDF
													</Button>
												)}
											</TableCell>
														...
														<>
															{order.status === 'pending' ? (
																<Button
																	size="sm"
																	variant="outline"
																	className="h-8 gap-1 border-emerald-500/30 text-emerald-600 hover:bg-emerald-50 hover:text-emerald-700 dark:border-emerald-500/20 dark:text-emerald-400 dark:hover:bg-emerald-500/10 cursor-pointer"
																	onClick={() => {
																		setOrderToRecibir(order);
																		setIsConfirmRecibirOpen(true);
																	}}
																...
																<Button
																	size="sm"
																	variant="outline"
																	className="h-8 gap-1 border-blue-500/30 text-blue-600 hover:bg-blue-50 hover:text-blue-700 dark:border-blue-500/20 dark:text-blue-400 dark:hover:bg-blue-500/10 cursor-pointer"
																	onClick={() => {
																		setOrderToReabrir(order);
																		setIsConfirmReabrirOpen(true);
																	}}
				...
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							...
							<strong>#{orderToDelete?.code}</strong>.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
			...
			>
				<AlertDialogContent className="z-[100] w-full max-w-[700px]">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							El documento de la orden de compra en formato PDF. Puede descargarla, imprimirla o visualizarla a continuación.
						</AlertDialogDescription>
					</AlertDialogHeader>
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							¿Está seguro de que desea marcar la orden de compra <strong>#{orderToRecibir?.code}</strong> como recibida? Esto actualizará el estado de los insumos.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
						<AlertDialogCancel onClick={() => setOrderToRecibir(null)}>Cancelar</AlertDialogCancel>
						<AlertDialogAction
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>Reabrir Orden de Compra</AlertDialogTitle>
						<AlertDialogDescription>
							¿Está seguro de que desea reabrir la orden de compra <strong>#{orderToReabrir?.code}</strong>? Esto cambiará su estado a pendiente.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
						<AlertDialogCancel onClick={() => setOrderToReabrir(null)}>Cancelar</AlertDialogCancel>
						<AlertDialogAction
