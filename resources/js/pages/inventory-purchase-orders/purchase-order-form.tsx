...
export default function PurchaseOrderForm({
	...
	const { data, setData, post, put, processing, errors, reset } = useForm({
		provider_id: purchaseOrder?.provider_id ? String(purchaseOrder.provider_id) : '',
	...
	const displayProviders = fetchedProviders.length > 0 ? fetchedProviders : providers;
	...
	const fetchProviders = (selectNewOne = false) => {
		fetch('/inventory-providers?all=true')
			...
			.then((data: Provider[]) => {
				if (selectNewOne) {
					const existingIds = new Set(displayProviders.map((p) => p.id));

					const newProvider = data.find((p) => !existingIds.has(p.id));
	...
	const fetchProducts = (selectNewOneForIndex?: number) => {
		fetch('/products?all=true')
			.then((res) => res.json())
			.then((data: Product[]) => {
				if (selectNewOneForIndex !== undefined && selectNewOneForIndex !== null) {
					const existingIds = new Set(fetchedProducts.map((p) => String(p.id)));
					const newProduct = data.find((p) => !existingIds.has(String(p.id)));

					if (newProduct) {
						handleProductChange(selectNewOneForIndex, 'product_id', String(newProduct.id));
					}
				}
	...
	const submitForm = () => {
		...
		const options = {
			onSuccess: () => {
				toast.success(purchaseOrder?.id ? 'Orden de compra actualizada correctamente' : 'Orden de compra creada correctamente');
				setIsSaving(false);
				onSuccess();
				reset();
			},
		...
		<>
			<form onSubmit={submit} className="space-y-6 px-5 py-4">
				...
				<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) => setData('date_requested', e.target.value)}
						/>
						<InputError message={errors.date_requested} />
					</div>

					<div className="space-y-2">
						<Label htmlFor="date_delivered">Fecha de Entrega (optional)</Label>
						<Input
							id="date_delivered"
							type="date"
							value={data.date_delivered}
							onChange={(e) => setData('date_delivered', e.target.value)}
						/>
						...
						{data.products.map((row, index) => (
							...
							>
								...
								<div className="flex-1 min-w-0">
									...
									/>
									<InputError message={errors[`products.${index}.specification`]} />
								</div>
								...
								<div className="flex justify-end shrink-0 md:pb-1">
									<Button
										type="button"
										variant="ghost"
										size="icon"
				...
				<div className="flex justify-end pt-4">
					<Button type="submit" disabled={processing} className="px-6">
						{purchaseOrder?.id ? 'Actualizar Orden' : 'Guardar Orden'}
					</Button>
				</div>
			</form>
			...
			<AlertDialog open={showConfirm} onOpenChange={setShowConfirm}>
				<AlertDialogContent className="max-w-[500px]">
					<AlertDialogHeader>
						<AlertDialogTitle>Resumen de Orden de Compra</AlertDialogTitle>
						<AlertDialogDescription>
							Revise detalladamente los insumos y el proveedor antes de guardar la orden.
						</AlertDialogDescription>
					</AlertDialogHeader>
					...
					<div className="grid gap-3 py-3 text-sm">
						<div className="flex justify-between border-b pb-2">
							...
							<span className="font-semibold text-foreground">
								{displayProviders.find(p => String(p.id) === String(data.provider_id))?.name || 'No seleccionado'}
							</span>
						</div>
						<div className="flex justify-between border-b pb-2">
							<span className="font-medium text-muted-foreground">Fecha Solicitada:</span>
							<span className="font-semibold text-foreground">
								{data.date_requested}
							</span>
						</div>
						<div className="flex justify-between border-b pb-2">
							...
							<span className="font-semibold text-foreground">
								{data.status === 'received' ? 'Recibida' : 'Pendiente'}
							</span>
						</div>
						...
						<div className="flex flex-col gap-1.5 border-b pb-2">
							...
							<div className="flex max-h-[150px] flex-col gap-1.5 overflow-y-auto pr-1">
								{data.products.map((row, idx) => {
									const prod = fetchedProducts.find(p => String(p.id) === String(row.product_id));
									return (
										<div key={idx} className="flex justify-between rounded border border-border/50 bg-muted/30 p-1.5 text-xs">
											<div className="flex flex-col">
												<span className="font-semibold">{prod?.name || 'Desconocido'}</span>
												<span className="text-[10px] text-muted-foreground italic">{row.specification || 'Sin especificación'}</span>
											</div>
											<div className="font-mono text-xs text-right">
												<span className="font-bold text-foreground">Cantidad: {row.quantity}</span>
											</div>
										</div>
									);
								})}
						...
						<div className="flex justify-between pt-2 text-base font-bold text-foreground">
							...
							<span>{data.products.reduce((acc, row) => acc + Number(row.quantity || 0), 0)}</span>
						</div>
					</div>

					<AlertDialogFooter>
						<AlertDialogCancel>Cancelar</AlertDialogCancel>
						<AlertDialogAction onClick={submitForm} className="bg-primary text-white hover:bg-primary/90">
							Confirmar y Guardar
						</AlertDialogAction>
					</AlertDialogFooter>
				</AlertDialogContent>
					...
					<div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center gap-4 bg-background/80 backdrop-blur-sm">
						...
						<div className="flex flex-col items-center text-center">
							...
							<p className="mt-1 max-w-xs text-sm text-muted-foreground">
								Estamos guardando la orden de compra y generando el archivo PDF. Por favor, espere.
							</p>
						</div>
					</div>,
					document.body,
