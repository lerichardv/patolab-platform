...
export default function ReferrerForm({
	...
	const [currentReferrer, setCurrentReferrer] = useState<Referrer | null>(referrer);
	...
	const [existingDuplicateReferrer, setExistingDuplicateReferrer] = useState<Referrer | null>(null);
	...
	useEffect(() => {
		setData({
			...
			referrer_type:
				...
				data.referrer_type ||
				'',
			phone: initialData?.phone || currentReferrer?.phone || data.phone || '',
			email: initialData?.email || currentReferrer?.email || data.email || '',
			address: initialData?.address || currentReferrer?.address || data.address || '',
			notes: initialData?.notes || currentReferrer?.notes || data.notes || '',
		});
			...
			fetch(`/referrers/${currentReferrer.id}/specimens?${queryParams}`, {
				...
				.then((resData) => {
					...
					if (resData.existing_duplicate) {
						setExistingDuplicateReferrer(resData.existing_duplicate);
			...
			>
				...
				<div className="flex justify-end gap-3 pt-4">
					...
					>
						...
						{currentReferrer ? 'Guardar Cambios' : 'Crear Remitente'}
					</Button>
				</div>
			</form>
						...
						<AlertDialogDescription asChild>
							<div className="space-y-4 pt-1 text-sm text-foreground/90">
								<p className="text-sm leading-relaxed">
									Ya existe un remitente registrado a nombre de{' '}
								...
								<div className="space-y-2 rounded-lg border border-blue-200 bg-blue-50/80 p-4 text-xs text-blue-950 dark:border-blue-900/50 dark:bg-blue-950/40 dark:text-blue-200">
									<p className="flex items-center gap-2 text-sm font-semibold text-blue-900 dark:text-blue-300">
										...
										¿Desea seleccionar este remitente existente?
									</p>
									<p className="leading-relaxed">
										Al seleccionar el remitente existente se cerrará este formulario sin modificar el remitente actual, y se elegirá automáticamente{' '}
										<strong>{existingDuplicateReferrer?.name} ({existingDuplicateReferrer?.notes})</strong> en el menú de remitentes de la muestra.
									</p>
								</div>
							</div>
						</AlertDialogDescription>
