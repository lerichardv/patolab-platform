...
class CreditGroupReportController extends Controller
{
    ...
    private function buildQuery(Request $request)
    {
        ...
        if ($request->has('specimen_type_id') && $request->get('specimen_type_id') !== 'all') {
            ...
            $typeIds = array_values(array_filter(array_map('strval', $typeIds), fn ($v) => $v !== '' && $v !== 'all' && $v !== 'none'));
            if (empty($typeIds)) {
                $query->whereRaw('1 = 0');
            } else {
        ...
        if ($request->has('examination_id') && $request->get('examination_id') !== 'all') {
            ...
            $examIds = array_values(array_filter(array_map('strval', $examIds), fn ($v) => $v !== '' && $v !== 'all' && $v !== 'none'));
            if (empty($examIds)) {
                $query->whereRaw('1 = 0');
            } else {
